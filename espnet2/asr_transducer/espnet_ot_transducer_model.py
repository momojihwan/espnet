...
class ESPnetASROTTransducerModel(AbsESPnetModel):
    ...
    def sinkhorn_knpp(self, a, b, cost_matrix, reg, max_iter=100, tol=0.05):
    ...
    def optimal_transport_alignment(
        ...
        with torch.no_grad():
            ...
            for i in range(B):
                transport_matrix[i] = self.sinkhorn_knpp(a[i:i+1].unsqueeze(-1), b[i:i+1].unsqueeze(-1), cost_matrix[i:i+1], 0.05)
