...
class ESPnetASRTransducerTuneModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 2-1. blank-frame-elimination
        # be_encoder_out, be_encoder_out_lens = self.blank_frame_elimination(
        #     encoder_out,
        #     target,
        #     t_len,
        #     u_len,
    ...
    def blank_frame_elimination(
        ...
        # ctc = CTC(
        #     odim=self.vocab_size, encoder_output_size=enc_out.size(2),
        # )
        ctc_loss = torch.nn.CTCLoss()
        self.ctc_lin.eval()
        with torch.no_grad():
            ctc_in = self.ctc_lin(encoder_out)
            emissions = torch.log_softmax(ctc_in.transpose(0, 1), dim=-1)

            target_mask = target != 0
            ctc_target = target[target_mask].cpu()
            print("emi : ", emissions.size())
            emission = emissions[0].cpu().detach()
            print("ctc_alignment : ", emission.size())
        ...
        # print("ctc : ", ctc_posterior.size())
        plt.imshow(emission.T)
        plt.colorbar()
        plt.title("Frame-wise class probabilities")
        plt.xlabel("time")
        plt.ylabel("Labels")
        plt.savefig("./ctc_post.png")
        exit()
        # logits = F.log_softmax(enc_out, dim=-1)
        # ctc_posterior = ctc.argmax(enc_out.cpu()).data.cpu().numpy()
        # ctc_posterior = torch.exp(logits).detach().cpu().numpy()
        
        return ctc_posterior
