...
class ESPnetASRAUXTransducerModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.aux_optim = torch.optim.SGD(teacher_model.parameters(), lr=0.001)
        self.maximization_steps = maximization_steps
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits_diff = self._calc_diff_logits(
            joint_out,
            teacher_joint_out
        )

        # 4-1. Calculate knowledge distillation loss
        loss_kd_1 = self._calc_kd_loss(
            logits_diff,
            teacher_joint_out
        )
        loss_kd_2 = self._calc_kd_loss(
            joint_out,
            logits_diff
        )

        loss_kd = (loss_kd_1 / 2) + (loss_kd_2 / 2)
    ...
    def _calc_diff_logits(
        ...
        for _ in range(self.maximization_steps):
            logits_diff = torch.norm(teacher_joint_out - joint_out, p=2)
            loss_max_diff = -logits_diff
            self.aux_optim.zero_grad()
            loss_max_diff.backward(retain_graph=True)
            self.aux_optim.step()
