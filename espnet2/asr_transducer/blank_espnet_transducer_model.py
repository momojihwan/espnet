...
class ESPnetASRTransducerModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_auxiliary_ctc:
            a = self.blank_frame_elimination(
                encoder_out,
                target,
                t_len,
                u_len,
    ...
    def blank_frame_elimination(
        ...
        with torch.no_grad():
            ...
            ctc_in = torch.softmax(ctc_in.transpose(0, 1), dim=-1)
            ctc_out = torch.argmax(ctc_in.transpose(0, 1), dim=-1)
            print("ctc out : ", ctc_out.size())
            print("ctc out : ", ctc_out)
            ctc_out = torch.nn.functional.one_hot(ctc_out, num_classes=2048)
            print("ctc out2 : ", ctc_out.size())
            print("ctc out2 : ", ctc_out)
            # exit()
            ctc_out = ctc_out.transpose(0, 1).cpu().detach()
            # print("ctc :" , ctc_in.size())
            # print("ctc :" , ctc_in)
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.imshow(ctc_out[0].T, cmap="Greens", aspect='auto', origin='lower', alpha=0.5)
            ax.set_xlabel('Input Time Step')
            ax.set_ylabel('Output Time Step')
            ax.set_xticks(range(len(ctc_out)))
            # ax.set_yticks(range(len(ctc_target)))
            plt.title("Frame-wise class probabilities")
            plt.savefig("./ctc_align.png")
            exit()
