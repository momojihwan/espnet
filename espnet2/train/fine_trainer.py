...
class Trainer:
    ...
    def encoder_resume(
        ...
        encoder_trim = {k:v for k, v in states.items() if k.startswith('encoder')}
        ctc_trim = {k:v for k, v in states.items() if k.startswith('ctc')}
        model.encoder.load_state_dict(encoder_trim)
        model.ctc.load_state_dict(ctc_trim)
        logging.info(f"The encoder training was resumed using {checkpoint}")
    ...
    def decoder_resume(
        ...
        print("states : ", states)
        print("states item : ", states["model"])
        model.load_state_dict(states["model"])
        decoder_trim = {k:v for k, v in states["model"].items() if k.startswith('decoder')}
        model.decoder.load_state_dict(decoder_trim)
        logging.info(f"The decoder training was resumed using {checkpoint}")
    ...
    def run(
        ...
        # else:
        #     print("0")
        #     if trainer_options.encoder_resume and (output_dir / "encoder.pth").exists():
        #         cls.encoder_resume(
        #             checkpoint=output_dir / "encoder.pth",
        #             model=model,
        #             ngpu=trainer_options.ngpu,
        #         )
        #     if trainer_options.decoder_resume and (output_dir / "decoder.pth").exists():
        #         cls.decoder_resume(
        #             checkpoint=output_dir / "decoder.pth",
        #             model=model,
        #             ngpu=trainer_options.ngpu,
        #         )
